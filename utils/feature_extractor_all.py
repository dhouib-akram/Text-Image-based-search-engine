...
df = pd.read_csv(
    "./final.csv", usecols=["ImageID", "OriginalURL", "Tags", "Author", "Title"]
)
...
for i, url in enumerate(
    df["OriginalURL"][last_processed_index:], start=last_processed_index
):
    ...
    if df10[df10["OriginalURL"] == url].empty:
        data = {
            ...
            "Title": df["Title"][i],
            "OriginalURL": url,
            "FeatureVector": json.dumps(features.tolist()),
        }
        df10 = df10._append(data, ignore_index=True)

    print(
        f"Processed URL {i + 1}/{len(df['OriginalURL'][last_processed_index:])}: {url}"
    )
